 ...
 */
function wp_publications_fetch_crossref_journal($issn, $mailto) {
    if (empty($issn)) {
        return array(
            'success' => false,
            'error' => __('No ISSN provided', 'wp-publications'),
        );
    }
    
    // Use the first ISSN if multiple are provided
    if (is_array($issn)) {
        $issn = reset($issn);
    }
    
    $url = sprintf(
        'https://api.crossref.org/journals/%s?mailto=%s',
        urlencode($issn),
        urlencode($mailto)
    );
    ...
    if ($code !== 200) {
        return array(
            'success' => false,
            'error' => sprintf(__('Journal API returned status code %d', 'wp-publications'), $code),
        );
    }
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        return array(
            'success' => false,
            'error' => __('Invalid JSON response from journal API', 'wp-publications'),
        );
    }
...
/**
 * Extract ISSN from work data
 */
function wp_publications_extract_issn($work_data) {
    if (isset($work_data['message']['ISSN']) && !empty($work_data['message']['ISSN'])) {
        return $work_data['message']['ISSN'];
    }
    
    if (isset($work_data['message']['issn-type'])) {
        $issns = array();
        foreach ($work_data['message']['issn-type'] as $issn_info) {
            if (isset($issn_info['value'])) {
                $issns[] = $issn_info['value'];
            }
        }
        if (!empty($issns)) {
            return $issns;
